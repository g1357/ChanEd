...
{
    /// <summary>
    /// Сериализатор для файлов в формате .scm. 
    /// </summary>
    public class ScmSerializer : SerializerBase
    {
        ...
        /// <summary>
        /// Буква модели телевизора
        /// </summary>
        private ModelConstants c;
        /// <summary>
        /// Список имён провайдеров сервиса
        /// </summary>
    ...
    #region ReadDvbsChannels()
        /// <summary>
        /// Считываем из файла данные о спуитниковых каналах.
        /// </summary>
        /// <param name="zip"></param>
        /// <param name="filename"></param>
        /// <param name="channels"></param>
        /// <param name="fileContent">Считанное из файла содерживое, ка массив байтов</param>
        /// <param name="entrySize">Размер записи о канале.</param>
        private void ReadDvbsChannels(string zip, string filename, ChannelList channels, out byte[] fileContent, int entrySize)
        {
            ...
            // Определить максимальную длину названия канала (из файла ini)
    ...
    private void ReadAstraHdPlusChannels(string zip)
    {
      ...
      for (int slotIndex = 0; slotIndex < count; slotIndex++)
      {
        ...
        private static byte[] ReadFileContent(string tempDir, string fileName)
        {
            ...
            // Если файл существует, возвращаем считанные из него байты, иначе - возвращаем null
            return File.Exists(path) ? File.ReadAllBytes(path) : null;
        }
        #endregion ReadFileContent()
